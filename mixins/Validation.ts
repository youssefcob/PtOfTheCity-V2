// Parses MM-DD-YYYY strictly; returns null for invalid dates
const parseMMDDYYYY = (value: any): Date | null => {
    ...
    constructor(public validationObject: FormValidation, public form: Form) {
        this.validator = {
            ...
            date: (arr?: string[]) => {
                ...
                if (arr && arr.length > 0) {
                    const firstElement = arr[0];
                    const parsedDate = parseMMDDYYYY(this.data);
                    if (parsedDate !== null) {
            ...
            match: (arr?: string[]) => {
                const pushError = (): void => {
                    if (this.element.message && this.element.message.match) {
                        this.errors.push({
                            [this.key]: this.element.message.match
                        });
                    } else {
                        this.errors.push({
                            [this.key]: `${this.key} does not match`
                        });
                    }
                }
                ...
                if (arr && arr.length > 0) {
                    const firstElement = arr[0];
                    let compareTo = this.form[firstElement as keyof typeof this.form];
            ...
            numeric: (arr?: string[]) => {
                const pushError = (): void => {
                    if (this.element.message && this.element.message.numeric) {
                        this.errors.push({
                            [this.key]: this.element.message.numeric
                        });
                    } else {
                        this.errors.push({
                            [this.key]: `${this.key} must be numeric`
                        });
                    }
                }
